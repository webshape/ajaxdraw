function runLoadTest () {
  test("Load Test", function () {
			...
			fs = s.read(doc);
			var p = fs._figures[0];
			equals(p.getBounds().start().x, "10", 'Polygon: starting x');
			equals(p.getBounds().start().y, "10", 'Polygon: starting y');
			equals(p.getBounds().end().x, "50", 'Polygon: ending x');
			equals(p.getBounds().end().y, "50", 'Polygon: ending y');
            equals(p.edgeNumber(3),"3",'Polygon: edges');
