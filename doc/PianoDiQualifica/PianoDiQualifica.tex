...
\item Analisi del codice tramite walkthrough alla ricerca di eventuali errori. Vista la particolare dispendiosit\`a di questa modalit\`a di verifica, essa verr\`a applicata unicamente a sezioni critiche del codice.
\end{elenconumerato}
...
La campagna di validazione si svolger\`a in due fasi distinte. In entrambe le fasi il test consister\`a nell'esecuzione dell'applicativo per controllare che quanto descritto nell'analisi dei requisiti, la quale ha valore contrattuale, sia presente nel prodotto. Il test sar\`a guidato dagli \textit{use case} presenti nell'analisi dei requisiti.
\begin{elenconumerato}[\textbf{}]{\subsubsecindent}
\item Alpha test: verr\`a eseguito internamente all'azienda.
\item Beta test: il prodotto verr\`a distribuito in modo limitato e controllato a persone esterne all'azienda.
\end{elenconumerato}
