...
\item[-] \textit{fallbackSelection(where)} metodo usato quando il browser non supporta \textit{getImageData} al posto di \textit{selectFigure}. Seleziona l'ultima figura (se esiste) il cui \textit{BoundingRectangle} contiene il punto \textit{where}. Per farlo itera lungo l'array \textit{_figures} a partire dalla fine, e quando trova un rettangolo che racchiude il punto si ferma e ritorna la figura corrispondente. Questo metodo \`e pi\`u impreciso rispetto a \textit{selectFigure}.
