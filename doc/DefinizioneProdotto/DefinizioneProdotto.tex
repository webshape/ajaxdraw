...
\paragraph{Metodi pubblici:}
\begin{elencopuntato}[\normindent]
\item[-] \textit{setAngle} setta l'angolo di rotazione \textit{\_rad}.
\item[-] \textit{transformPoints} effettua una trasformazione sulla figura selezionata nel canvas, ritorna la modifica dei vertici della figura ottenuti attraverso il metodo\textit{getMainPoints}; la figura viene ridisegnata con l'ausilio del metodo \textit{draw(canvas)}. 
