...
%%%%%%%%%%% PARTE DA MODIFICARE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
        ...
        \hline
		1.6 &    21$\slash$03$\slash$2009 Inserito diagramma delle classi della parte Server & Dal Bosco Davide \\
		\hline
		1.5 &    21$\slash$03$\slash$2009 Correzioni varie per metodi elementi GUILogic & Bizzotto Piero \\
		\hline	
...
\leftskip=20pt Si vedano le \textit{NormeDiprogetto}, versione 3.0.
\newpage
...
\subsubsezione{Tipo, obiettivo e funzione del componente}
Classe concreta che rappresenta lo specifico pulsante che si occupa di salvare il set di figure attualmente disegnato sul canvas, integrato nella sezione "File" della barra in alto.
\subsubsezione{Relazioni d'uso di altre componenti}
Nessuna.
\subsubsezione{Interfacce e relazioni di uso da altre componenti}
Nessuna.
...
\paragrafo{Metodi pubblici:}
\begin{elencopuntato}[\subsubsecindent]
\item[-]  \textit{bindCanvas(toolbar,canvas,canvasObj,visual,figureSet)} overriding del metodo ereditato da \textit{Button}, collega il canvas all'azione di selezione di una figura disegnata sul canvas, e consente quindi di attivare le funzionalit\` a di spostamento della figura stessa, del suo ridimensionamento (tramite \textit{{\_}handleCtrlPoint}), di clonazione ed eliminazione, tramite i metodi descritti in \textit{Visualization} e dell'impostazione del  livello di  profondit\`a, tramite gli appositi metodi \textit{toTop} e \textit{toBottom};
\item[-] \textit{{\_}handleCtrlPoints} metodo privato, che gestisce il click del mouse verificando dove questo avvenga: nel caso l'area cliccata sia un punto di controllo di una figura disegnata, collega il trascinamento del mouse (con il pulsante sinistro premuto) al ridimensionamento di quest'ultima, e nel caso delle linee a mano libera consente di spostare anche i punti che le compongono.
\end{elencopuntato}
...
Estende \textit{Button}.
\subsubsezione{Interfacce e relazioni di uso da altre componenti}
Viene estesa da \textit{ZoomInButton} e \textit{ZoomOutButton}.
\subsubsezione{Attivit\`a svolte e dati trattati}
\leftskip=36pt Nessuna.

\subsezione{ZoomInButton}
\subsubsezione{Tipo, obiettivo e funzione del componente}
Classe concreta che rappresenta un pulsante che attiva le funzionalit\` a di ingrandimento dell'area di visualizzazione del canvas.
\subsubsezione{Relazioni d'uso di altre componenti}
Estende \textit{ZoomButton}.
\subsubsezione{Interfacce e relazioni di uso da altre componenti}
\textit{Toolbar} ne mantiene un' istanza, e utilizza \textit{Scale} per ottenere e reimpostare il valore del fattore di riscalamento.
\subsubsezione{Attivit\`a svolte e dati trattati}
\paragrafo{Metodi pubblici:}
\begin{elencopuntato}[\subsubsecindent]
\item[-] \textit{bindCanvas(toolbar,canvas,canvasObj,visual,figureSet)} collega l'azione di ingrandimento dell'area visualizzata al canvas. L'evento click prende il punto al centro dell'area mostrata, e in base al fattore di riscalamento attuale aumenta il livello zoom del 50 per cento mantenendo tale punto come riferimento.
\end{elencopuntato}
...
\subsubsezione{Relazioni d'uso di altre componenti}
Estende \textit{ZoomButton}, e utilizza \textit{Scale} per ottenere e reimpostare il valore del fattore di riscalamento.
\subsubsezione{Interfacce e relazioni di uso da altre componenti}
\textit{Toolbar} ne mantiene un' istanza.
\subsubsezione{Attivit\`a svolte e dati trattati}
\paragrafo{Metodi pubblici:}
\begin{elencopuntato}[\subsubsecindent]
\item[-] \textit{bindCanvas(toolbar,canvas,canvasObj,visual,figureSet)} collega l'azione di rimpicciolimento dell'area visualizzata al canvas. L'evento click prende il punto al centro dell'area mostrata, ed in base al fattore di riscalamento attuale diminuisce il livello zoom del 50 per cento, mantenendo tale punto come riferimento. Ovviamente il livello \`e limitato al doppio dell'ingrandimento iniziale, sia per evitare numeri minori o uguali a 0, sia perch\` e una volta che tutta l'area di disegno \` e visualizzata non sono necessari ultieriori rimpicciolimenti. 
\end{elencopuntato}
...
\subsubsezione{Interfacce e relazioni di uso da altre componenti}
\textit{Visualization}, \textit{ZoomInButton} e \textit{ZoomOutButton} ne utilizzano un'istanza.
\subsubsezione{Attivit\`a svolte e dati trattati}
\paragrafo{Attributi privati:}
\begin{elencopuntato}[\subsubsecindent]
...
\paragrafo{Metodi pubblici:}
\begin{elencopuntato}[\subsubsecindent]
\item[-] \textit{getBuilder()} ritorna il tipo di oggetto disegnato tramite la pressione di questo pulsante, per permetterne il riconoscimento al metodo \textit{bindCanvas} in modo da associare correttamente la figura da disegnare al suo pulsante. Quest'ultimo metodo citato contiene un controllo che imposter\` a, visto che la figura da disegnare sar\` a un poligono, il numero di lati ricavandolo dall'apposito campo del dialog delle propriet\` a. 
\end{elencopuntato}
...
\begin{elencopuntato}[\subsubsecindent]
\item[-] \textit{getBuilder()} ritorna il tipo di oggetto disegnato tramite la pressione di questo pulsante, per permetterne il riconoscimento al metodo \textit{bindCanvas} in modo da associare correttamente la figura da disegnare al suo pulsante.
\item[-]  \textit{bindCanvas(toolbar,canvas,canvasObj,visual,figureSet,borderColor)} overriding del metodo ereditato da \textit{Button}, collega il canvas all'azione di disegno di una linea a mano libera. Il principio di funzionamento consiste nel creare di seguito una serie di piccole curve di Bezier ogniqualvolta il cursore del mouse si sposti (con il pulsante sinistro in pressione) verso una posizione sul canvas distante 10 pixel (sia in verticale che in orizzontale) dal precedente punto di controllo, in modo da garantire una certa fluidit\` a nel disegno.
\end{elencopuntato}
...
\begin{elencopuntato}[\subsubsecindent]
\item[-] \textit{getBuilder()} ritorna il tipo di oggetto disegnato tramite la pressione di questo pulsante, per permetterne il riconoscimento al metodo \textit{bindCanvas} in modo da associare correttamente la figura da disegnare al suo pulsante.
\item[-]  \textit{bindCanvas(toolbar,canvas,canvasObj,visual,figureSet,borderColour,fillColor)} overriding del metodo ereditato da \textit{Button}, collega il canvas all'azione di disegno di una casella di testo. L'overriding \` e reso necessario dai parametri specifici da impostare per il disegno della casella, quali la stringa, il colore e il tipo di carattere da utilizzare.
\end{elencopuntato}
...
\subsubsezione{Relazioni d'uso di altre componenti}
Legge e modifica \textit{BoundingRectangle}.
\subsubsezione{Interfacce e relazioni di uso da altre componenti}
Nessuna.
\subsubsezione{Attivit\`a svolte e dati trattati}
\paragrafo{Metodi pubblici:}
\begin{elencopuntato}[\subsubsecindent]
\item[-] \textit{BoundingRectangleSetter(bounds)} costruttore. Prende un \textit{BoundingRectangle} e setta il valore dei campi di un dialog statico implementato in HTML. I campi contengono: i valori x ed y dell'inizio di \textit{bounds} e i valori di altezza e larghezza. Collega un evento al pulsante di conferma del dialog, e se l'utente ha modificato i valori, e i valori inseriti sono dei numeri, modifica le corrispondenti propriet\`a di \textit{bounds} ed esegue un refresh.
\end{elencopuntato}
...
\item[-] \textit{drawSelection(canvas)} disegna dei quadratini di colore blu e con opacit\`a 0.8 intorno ai punti principali della figura, ottenuti tramite \textit{getMainPoints} e traccia i bordi del \textit{BoundingRectangle} di colore grigio con opacit\`a 0.8, solo se la figura \`e selezionata. Sia per disegnare i quadrati, sia per disegnare i bordi, utilizza il metodo \textit{strokeRect} del contesto grafico;
...
Creato dalla lettura di un file SVG oppure dall'interfaccia utente. Manipolato dall'interfaccia utente. Chiamato da \textit{SVGWriter} per trasformare la figura in formato SVG, e da \textit{SVGReader} per creare un'istanza da un file SVG.
\subsubsezione{Attivit\`a svolte e dati trattati}
Nel costruttore viene controllato se il browser supporta le API per il disegno del testo. In caso contrario, il metodo \textit{draw} viene settato a \textit{fallbackDraw}.
\paragrafo{Attributi privati:}
\begin{elencopuntato}[\subsubsecindent]
\item[-] \textit{{\_}fillColour} colore di riempimento, di tipo \textit{FillColour}.
...
Nessuna.
\subsubsezione{Interfacce e relazioni di uso da altre componenti}
Usato da \textit{SVGWriter} per ottenere e ritornare la stringa rappresentate il documento SVG.
\subsubsezione{Attivit\`a svolte e dati trattati}
\paragrafo{Attributi privati:}
\begin{elencopuntato}[\subsubsecindent]
...
\subsezione{ParsingError}
\subsubsezione{Tipo, obiettivo e funzione del componente}
Si occupa dei controlli sulla presenza degli attributi e sui loro valori, avvisando in caso di errore.
\subsubsezione{Relazioni d'uso di altre componenti}
Nessuna.
\subsubsezione{Interfacce e relazioni di uso da altre componenti}
...
\begin{figure}[!ht]
\centering
\includegraphics[scale=0.5]{ServerClasses.png}
\caption{Classi e loro relazioni - Server}
\end{figure}
