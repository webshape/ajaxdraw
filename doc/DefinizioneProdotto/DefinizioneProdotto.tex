...
%%%%%%%%%%%%%%FINE INTESTAZIONE COLONNE%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		 \hline
%%%%%%%%%%% PARTE DA MODIFICARE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%		
    	 	0.2 &    18$\slash$02$\slash$2009 descrizione classe Rotation e metodo clone nella classe figure & Rizzo Maurizio \\
                \hline
...
\\item[-] \textit{clone()} metodo che utilizza \textit{getMainPoints} per ottenere i punti della figura selezionata, propone sul canvas una copia dell'originale con l'ausilio del metodo \textit{draw(canvas)}; il clone dell'originale viene disegnato spostato 10 10px verso l'alto o in relazione a dove si trova la figura origine sul canvas.
\end{elencopuntato}
...
\paragraph{Metodi pubblici:}
\begin{elencopuntato}[\normindent]
\item[-] \textit{getCentre} permette di ottenere il centro della figura utilizzando il metodo pubblico \textit{Centre()} della classe \textit{BoundingRectangle}.
\item[-] \textit{setPoints} ritorna la modifica dei vertici della figura ottenibili attraverso \textit{getMainPoints}; la figura viene ridisegnata con l'ausilio del metodo \textit{draw(canvas)} a partire dai punti ottenuti dalla rotazione.  
