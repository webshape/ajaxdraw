...
\paragrafo{Metodi pubblici:}
\begin{elencopuntato}[\normindent]
\item[-] \textit{refresh()} ridisegna il set di figure sul canvas;
...
\item[-] \textit{getClickCoordsWithinTarget(event)} ritorna il punto esatto del canvas sul quale \`  e avvenuto il click  del mouse.
\end{elencopuntato}
...
\paragrafo{Metodi pubblici:}
\begin{elencopuntato}[\normindent]
\item[-] \textit{getId()} ritorna  \textit{{\_}fid};
\item[-] \textit{clear()} cancella tutte le figure disegnate sul canvas;
...
\item[-] \textit{clone(figure)} Riceve tutte le propriet\`a della figura selezionata passate dal parametro attuale \textit{figure}; invoca il metodo \textit{getMainPoints} da cui preleva i vertici della figura. Costruisce e ritorna una figura clone dell' originale le cui coordinate sul canvas sono spostate di 10px in relazione alla posizione della figura di partenza. 
\end{elencopuntato}
...
\item[-] \textit{transformPoints} effettua una trasformazione sulla figura selezionata nel canvas, ritorna la modifica delle coordinate dei vertici della figura ottenuti attraverso il metodo\textit{getMainPoints}; la figura viene ridisegnata con l'ausilio del metodo \textit{draw(canvas)}. 
\item[-] \textit{applyToContext(context)} setta l'attributo \textit{angle} del contesto grafico alla propria rappresentazione sul canvas, e richiama il metodo \textit{applyToContext} su \textit{{\_}rad} passando \textit{context} come argomento.
...
\subsubsezione{Tipo, obiettivo e funzione del componente}
Si occupa di chiamare il metodo \textit{fromSVG} su ogni tag SVG e ritornare il documento che corrisponde alla situazione corrente nel file SVG.
\subsubsezione{Relazioni d'uso di altre componenti}
Si serve di \textit{XMLParser} per fare il parsing del documento come XML, e per ogni comando SVG viene creato un tag SVG che lo rappresenta.
Si serve di \textit{SVGElementRegistry} per creare la figura dal tag SVG e ritornarla, utilizzando il metodo \textit{makeFigureClassFromTag} per iterare attraverso la collezione di tag SVG.
\subsubsezione{Interfacce e relazioni di uso da altre componenti}
Si serve di \textit{SVGReadable}
\subsubsezione{Attivit\`a svolte e dati trattati}
\paragrafo{Metodi pubblici:}
\begin{elencopuntato}[\normindent]
\item[-]  \textit{read()} chiama il metodo \textit{fromSVG} su ogni tag SVG e ritorna il documento con le figure corrispondenti.
\end{elencopuntato}

\subsezione{XMLParser}
\subsubsezione{Tipo, obiettivo e funzione del componente}
Si occupa di fare il parsing del documento SVG come XML.
\subsubsezione{Relazioni d'uso di altre componenti}
Nessuna
\subsubsezione{Interfacce e relazioni di uso da altre componenti}
Usato da \textit{SVGReader} per realizzare figure dai tag SVG.
\subsubsezione{Attivit\`a svolte e dati trattati}
\paragrafo{Metodi pubblici:}
\begin{elencopuntato}[\normindent]
\item[-]
...
\subsubsezione{Tipo, obiettivo e funzione del componente}
Si occupa di creare la figura dal tag SVG e ritornarla.
\subsubsezione{Relazioni d'uso di altre componenti}
Nessuna
\subsubsezione{Interfacce e relazioni di uso da altre componenti}
Usato da \textit{SVGReader} per realizzare il caricamento.
\subsubsezione{Attivit\`a svolte e dati trattati}
\paragrafo{Metodi pubblici:}
\begin{elencopuntato}[\normindent]
\item[-]
