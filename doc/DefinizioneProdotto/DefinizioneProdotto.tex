...
Classe concreta che rappresenta la pagina \underline{web} visualizzata dall'utente tramite l'internet \underline{browser}, contenitore di tutti i widgets componenti l'interfaccia.
...
Gestisce completamente le figure: il disegno, la conversione da e verso \underline{SVG}, le loro propriet\`a, la loro manipolazione e creazione. Utilizza le interfacce messe a disposizione dallo strato server e offre le proprie funzionalit\`a allo strato GUI. Nel grafico in figura 1 si riportano le classi per la gestione delle figure con le loro relazioni. Nel diagramma non si mostrano attributi e metodi in quanto ampiamente descritti testualmente in seguito.
...
\paragrafo{Metodi pubblici:}
\begin{elencopuntato}[\subsubsecindent]
\item[-]  \textit{write()} chiama il costruttore \textit{SVGGenerator} ottenendo cos\`i il documento sotto forma di stringa con l'header SVG. Poi chiama il metodo \textit{toSVG} su ogni figura e ritorna il documento SVG con il metodo \textit{flush} di \textit{SVGGenerator}.
\end{elencopuntato}
