...
\end{elencopuntato}
N.B. Alcune funzionalit\`a avanzate, come il cambiamento del tipo di carattere di una casella di testo, sono attualmente supportate solamente dalle versioni non stabili dei succitati browsers; saranno pertanto disabilitate dove non sono disponibili.
...
\caption{cambio coordinate e dimensioni manuale - PRIMA}
\end{figure}
...
\vspace{100pt}
Qui \`e stata selezionata la funzione di cambiamento delle coordinate spaziali all'interno dell'area di disegno, allo stesso tempo \`e stata cambiata anche la dimensione di altezza e larghezza della figura. tutte le modifiche suddette sono effettuabili sulla finestra \textit{Modifica Attributi} modificando i parametri appositi rispettivamente X e Y per la posizione sul piano di disegno, H e L per altezza e larghezza dell'oggetto.
\newpage
