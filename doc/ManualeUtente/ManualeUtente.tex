...
%%%%%%%%%%% PARTE DA MODIFICARE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          ...
          \hline
          1.1 & 20/03/09 Aggiunta funzione cambiamento di livello & Rizzo Maurizio \\
          \hline
...
\begin{elencopuntato}[\normindent]
 ...
 \item[-]   Internet Explorer 7 o superiore;
...
\subsubsezione{Lato client}
\` E necessario il chiarimento di qualche dettaglio prima di utilizzare tutte le potenzialit\`a del sistema. Alcuni browsers necessitano infatti di alcuni accorgimenti per permettere all'utente di usufruire in pieno dell'esperienza AJAXDRAW. In particolare:
\begin{elencopuntato}[\normindent]
...
per funzionare. Il programma \`e compatibile con le versioni 7 e 8 del browser, salvo alcuni problemi con lo spostamento dell'area di visualizzazione e lo zoom, e altri di piccola entit\` a. 
\end{elencopuntato}
\subsubsezione{Lato server}
...
\textit{$ < $/tomcat-users$ > $} \\
\\
In questo modo sono stati impostati uno Username  \textit{admin} e una Password  \textit{admin}. Queste credenziali serviranno per accedere al Tomcat Manager.\\
A questo punto avviare un browser e digitare il seguente indirizzo\\ \href{http://localhost:8080/manager/html}{http://localhost:8080/manager/html}\\
