...
Mantiene un'istanza della classe \textit{BoundingRectangle} per rappresentare i limiti della figura. Contiene anche un'istanza di \textit{Colour} che rappresenta il colore del bordo, e un'istanza di \textit{Opacity} che rappresenta l'opacit\`a della figura.
\subsubsezione{Interfacce e relazioni di uso da altre componenti}
Offre un metodo astratto \textit{draw} per disegnare la figura sul canvas, e un metodo \textit{getMainPoints} che verr\`a usato per evidenziare i punti della figura manipolabili dall'utente.
\subsubsezione{Attivit\`a svolte e dati trattati}
Gestisce le propriet\`a comuni a qualsiasi figura, e permette di selzionare o deselezionare la figura.

\subsezione{FigureSet}
\subsubsezione{Tipo, obiettivo e funzione del componente}
Rappresenta un collezione di figure.
\subsubsezione{Relazioni d'uso di altre componenti}
Contiene le istanze di \textit{Figure} correntemente attive.
\subsubsezione{Interfacce e relazioni di uso da altre componenti}
Permette si selezionare una figura a partire da un punto tramite il metodo \textit{selectFigure}. Viene selezionata la figura che contiene quel punto, se esiste. Permette di applicare un'operazione ad ogni figura tramite il metodo\textit{each}. Implementa \textit{SVGWritable}.
\subsubsezione{Attivit\`a svolte e dati trattati}
Gestisce le figure attive, permettendone l'aggiunta, la selezione o la cancellazione.
